 ...
 */
package com.dataagility.ICAN.BHLibHEALTHLINK;
 ...
 * @author da-admin
 */
public class HEALTHLINKCERNERProcessFromUFD extends ProcessSegmentsFromUFD {
...
//    public String mHEALTHLINKType = "";
     ...
     */
    public HEALTHLINKCERNERProcessFromUFD(String pHL7Message, String pEnvironment) throws ICANException {
        super(pHL7Message);
...
// HEALTHLINK ADT Structure is ... MSH, PID, PV1
        ...
        int aHealthLinkDoctorNumberCERNER = 0;
        String aOBR_16Value = "";
        String aOBR_28Value = "";
        HL7Segment aInOBRSegment;
        aInOBRSegment = new HL7Segment(mInHL7Message.getSegment(HL7_23.OBR));
        if (aSendingApp.startsWith("CERNER")) {
            //copy OBR_16 to OBR_28 for CERNER results
        ...
        if (aOBR_16Value.length() > 0 || aOBR_28Value.length() > 0) {
            aHealthLinkDoctorNumberCERNER = aInOBRSegment.countRepeatFields(HL7_23.OBR_28_Results_Copies_To);
        }
        if (aSendingApp.startsWith("GERIS")) {
            mHEALTHLINKMessageArray = new String[aHealthLinkDoctorNumber + 2];
        } else {
            mHEALTHLINKMessageArray = new String[aHealthLinkDoctorNumberCERNER + 2];
        }
        if (mInHL7Message.isEvent("R01, R03")) {
            //mInOBRSegment = new HL7Segment(mInHL7Message.getSegment(HL7_23.OBR));
            if (aSendingApp.startsWith("CERNER") && aHealthLinkDoctorNumberCERNER > 0) {
                mHLCount = 1;
                ...
                for (int aHLDoctorCount = 1; aHLDoctorCount <= aHealthLinkDoctorNumberCERNER; aHLDoctorCount++) {
...
//Check and swap ALFUR/CGMCUR/SDMHUR to ALF
        ...
        aPIDSegOUT.set(HL7_23.PID_10_race, "9");                    // Force to "Unknown" since CERNER does not send this.
...
//--------------------------------------------------------------------------
    ...
    public HL7Group processReqDets_FromUFD() {
        ...
        String aGroupID[] = (mInHL7Message.countSegments("ORC") != 0) ? HL7_23.Group_Orders : HL7_23.Group_Orders_without_ORC;
        int aRequestDetsCount = mInHL7Message.countGroups(aGroupID);
        for (int i = 1; i <= aRequestDetsCount; i++) {
            aInReqDetsGroup = new HL7Group(mInHL7Message.getGroup(aGroupID, i));

            //process ORC if exists modified 7may14 by gg
            if (aGroupID[0].equals("ORC")) {
