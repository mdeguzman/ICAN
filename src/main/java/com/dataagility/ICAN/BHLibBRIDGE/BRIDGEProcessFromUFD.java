/*
 * BRIDGEProcessFromUFD.java
 *
 * Created on 11 October 2005, 15:25
 *
 */

package com.dataagility.ICAN.BHLibBRIDGE;
 ...
 * @author fillinghamr
 */
public class BRIDGEProcessFromUFD extends ProcessSegmentsFromUFD {
     ...
     */
    public BRIDGEProcessFromUFD(String pHL7Message, String pEnvironment) throws ICANException {
...
// Make certain we do not process CERNER back to CERNER ------------------------
        try {
            if (aMSHSegment.get(HL7_23.MSH_3_sending_application).indexOf("BRIDGE") >= 0 ||
...
// ADT A17 messages .... -------------------------------------------------------
        if (mInHL7Message.isEvent("A17") && hasValue(HL7_23.MSH_9_1_message_type, "ADT")) {
            aOutMess1.append(processMSHFromUFD("BRIDGE"));
...
// Process a ORM message for PARIS or NMHL7
    public HL7Message processORM_FromUFD() throws ICANException {
        ...
        aMess.append(processMSHFromUFD("BRIDGE"));
...
// Process a ORU message for PARIS or NMHL7
    public HL7Message processORU_FromUFD() throws ICANException {
        ...
        HL7Message aMess = new HL7Message(k.NULL);
        HL7Segment aPIDSegment = processORM_ORU_PIDFromUFD();
        HL7Segment aMSHSegment = processMSHFromUFD("BRIDGE");
        HL7Group aNTEGroup = processNTEs_FromUFD();
...
//------------------------------------------------------------------------------
    ...
    public HL7Segment processZPIFromUFD() throws ICANException {
        ...
        aZPIOut.set(BRIDGE_23.ZPI_8_Family_Doctor, getXCNFromZBX("VISIT", "GPCode"));

        if (aHarpFlag.equalsIgnoreCase("HARP") || aHarpFlag.equalsIgnoreCase("CHORD")) {
            aZPIOut.set(BRIDGE_23.ZPI_13_Church, aHarpFlag);
        } else {
            if (aHL7Mess.isEvent("A01, A08, A28, A31")) {
                aZPIOut.set(BRIDGE_23.ZPI_13_Church, "NULL");
            }
        }
//
// Sort out the SDMH 6/7 digit Patient UR bits
        ...
        if (aUR.length() == 6) {
            aZPIOut.set(BRIDGE_23.ZPI_15_Species, "S");
        } else {
            aZPIOut.set(BRIDGE_23.ZPI_15_Species, "U");
        }
        return aZPIOut;
    }
...
//------------------------------------------------------------------------------
    ...
    public HL7Group processZBXSegmentsFromUFD() throws ICANException {
        ...
        //handle pathnet medicare expiry date eg if 201705, convert to 05/2017 otherwise keeps the original
        if (!aOBXValue.equals(k.NULL)) {
            if (aOBXValue.length() == 6) {
                aOBXValue = aOBXValue.substring(4) + "/" + aOBXValue.substring(0, 4);
            }
...
//
    public HL7Segment processFILMBAGMSHFromUFD() {
        ...
        aOutMSHSegment.set(HL7_23.MSH_5_receiving_application, "BRIDGE");
