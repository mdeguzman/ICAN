/*
 * STOCCAProcessFromUFD.java
 *
 * Created on 11 October 2005, 17:04
 *
 */

package com.dataagility.ICAN.BHLibSTOCCA;

import com.dataagility.ICAN.BHLibClasses.*;

/**
 * STOCCAProcessFromUFD contains the methods required to build a STOCCA message
 * from a UFD HL7 message structure
 * @author Ray Fillingham and Norman Soh
 */
public class STOCCAProcessFromUFD extends ProcessSegmentsFromUFD {
    /**
     * Constant class
     */
    ...
    //--------------------------------------------------------------------------
    /**
     * This constructor creates a new instance of STOCCAProcessFromUFD passing a HL7 UFD
     * message structure
     * @param pHL7Message HL7 message text string
     * @throws BHLibClasses.ICANException Throws ICANException Exception class
     */
    public STOCCAProcessFromUFD(String pHL7Message, String pEnvironment) throws ICANException {
        super(pHL7Message);
    ...
    public String[] processMessage() throws ICANException {
        ...
        mVersion = "A";
        String aSTOCCAMessageArray[] = {k.NULL, k.NULL, k.NULL};
        HL7Message aOutHL7Message = new HL7Message();
        mInHL7Message = new HL7Message(mHL7Message, k.TRIM_LAST);

        HL7Segment aMSHSegment = processMSHFromUFD("STOCCA");
        HL7Segment aEVNSegment = processEVNFromUFD();
        HL7Segment aPIDSegment = processPIDFromUFD();

        aMSHSegment = processSTOCCA_MSH(aMSHSegment);
        aEVNSegment = processSTOCCA_EVN(aEVNSegment);
        aPIDSegment = processSTOCCA_PID(aPIDSegment);
        ...
        if (mInHL7Message.isEvent("A01, A02, A03, A04, A08, A12, A13, A28, A31") &&
            ...
            HL7Segment aZPDSegment = new HL7Segment(STOCCA_231.ZPD);
            ...
            aZPDSegment = processSTOCCA_ZPD(aZPDSegment, aPV1Segment);
            aPV1Segment = processSTOCCA_PV1(aPV1Segment, aPV2Segment);
            aGT1Group = processSTOCCA_GT1(aGT1Group, aMSHSegment);
            ...
            aSTOCCAMessageArray[0] = aMSHSegment.getField(HL7_23.MSH_3_sending_application);
            aSTOCCAMessageArray[1] = aMSHSegment.getField(HL7_23.MSH_4_sending_facility);
            aSTOCCAMessageArray[2] = aOutHL7Message.getMessage();
        ...
        return aSTOCCAMessageArray;
    }
    //--------------------------------------------------------------------------
    /**
     * STOCCA specific processing for MSH segment
     * @param aMSHSegment MSH segment class object
     * @return MSH segment class object
     * @throws BHLibClasses.ICANException Throws ICANException Exception class
     */
    public HL7Segment processSTOCCA_MSH(HL7Segment aMSHSegment) throws ICANException {
    ...
    //--------------------------------------------------------------------------
    /**
     * STOCCA specific processing for EVN segment
     * @param aEVNSegment EVN segment class object
     * @return EVN segment class object
     * @throws BHLibClasses.ICANException Throws ICANException Exception class
     */
    public HL7Segment processSTOCCA_EVN(HL7Segment aEVNSegment) throws ICANException {

        return aEVNSegment;
    }
    //--------------------------------------------------------------------------
    /**
     * STOCCA specific processing for PID segment
     * @param aPIDSegment PID segment class object
     * @return PID segment class object
     * @throws BHLibClasses.ICANException Throws ICANException Exception class
     */
    public HL7Segment processSTOCCA_PID(HL7Segment aPIDSegment) throws ICANException {
        ...
        if (aDVACardType.length() > 0) {
            CodeLookUp aLookUpValue = new CodeLookUp("STOCCA_DVA_CardType.table", mEnvironment);
    ...
    //--------------------------------------------------------------------------
    /**
     * STOCCA specific processing for ZPD segment
     ...
     */
    public HL7Segment processSTOCCA_ZPD(HL7Segment aZPDSegment, HL7Segment aPV1Segment) throws ICANException {
        String aDate = k.NULL;
        ...
        if (aPV1Segment.isEmpty(HL7_23.PV1_10_hospital_service) ||
            ...
            if (aPV1Segment.get(HL7_23.PV1_2_patient_class).equalsIgnoreCase("E")) {
                //For eastern, MERLIN will replace STOCCA - so the following if syntax should be amended later
                if (mFacility.matches("ANG|BHH|MAR|PJC")) {
                    aZPDSegment.set(STOCCA_231.ZPD_2_patient_team, "EMER");
                } else {
                    aZPDSegment.set(STOCCA_231.ZPD_2_patient_team, mFacility.concat("-EMER"));
                }
            } else {
                aZPDSegment.set(STOCCA_231.ZPD_2_patient_team, "\"\"");
            }
        } else {
            //For eastern, MERLIN will replace STOCCA - so the following if syntax should be amended later
            if (mFacility.matches("ANG|BHH|MAR|PJC")) {
                aZPDSegment.set(STOCCA_231.ZPD_2_patient_team, aPV1Segment.get(HL7_23.PV1_10_hospital_service));
            } else {
                aZPDSegment.set(STOCCA_231.ZPD_2_patient_team, mFacility.substring(0,1).concat("-").concat(aPV1Segment.get(HL7_23.PV1_10_hospital_service)));
            }
            //add claim number to ZPD_1
            String aClaimNumber = getFromZBX("FINANCE", "CLAIM_NUMBER");
            aZPDSegment.set(STOCCA_231.ZPD_1_claim_number, aClaimNumber);
        }
        ...
        if (aDate.length() > 0) {
            ...
            aZPDSegment.set(STOCCA_231.ZPD_7_Medicare_Exp_Date, aDate);
            if (!aDate.equalsIgnoreCase(k.NULL)) {
                if (aDate.length() == 6) {
                    String aReg1 = aDate.substring(4, 6);
                    if (aReg1.equalsIgnoreCase("02")) {
                        aZPDSegment.set(STOCCA_231.ZPD_7_Medicare_Exp_Date, aDate.concat("28"));
                    } else if (aReg1.equalsIgnoreCase("04") ||
                            aReg1.equalsIgnoreCase("06") ||
                            aReg1.equalsIgnoreCase("09") ||
                            aReg1.equalsIgnoreCase("11")) {
                        aZPDSegment.set(STOCCA_231.ZPD_7_Medicare_Exp_Date, aDate.concat("30"));
                    } else {
                        aZPDSegment.set(STOCCA_231.ZPD_7_Medicare_Exp_Date, aDate.concat("31"));
                    }
                }
            }
        }
        ...
        if (aDate.length() > 0) {
            aZPDSegment.set(STOCCA_231.ZPD_9_HCC_Pension_Exp_Date, aDate);
        }
    ...
    //--------------------------------------------------------------------------
    /**
     * STOCCA specific processing for PV1 segment
     * @param aPV1Segment PV1 segment class object
     * @return PV1 segment class object
     * @param aPV2Segment PV2 segment class object
     * @throws BHLibClasses.ICANException Throws ICANException Exception class
     */
    public HL7Segment processSTOCCA_PV1(HL7Segment aPV1Segment, HL7Segment aPV2Segment) throws ICANException {
        ...
        if (!aPV1Segment.getSegment().equalsIgnoreCase(k.NULL)) {
            ...
            if (mFacility.matches("ANG|BHH|MAR|PJC")) {
                ...
                CodeLookUp mLook = new CodeLookUp("CERNER_PayClass_STOCCA.table", mEnvironment);
    ...
    //--------------------------------------------------------------------------
    /**
     * STOCCA specific processing for GT1 segment
     * @param aGT1Group GT1 group class object
     * @param aMSHSegment MSH segment class object
     * @throws BHLibClasses.ICANException Throws ICANException Exception class object
     * @return GT1 group class object
     */
    public HL7Group processSTOCCA_GT1(HL7Group aGT1Group, HL7Segment aMSHSegment) throws ICANException {
    ...
    //--------------------------------------------------------------------------
    /**
     * STOCCA specific processing for PID segment (overiding generic version)
     * @throws BHLibClasses.ICANException Throws ICANException Exception class object
     * @return PID segment class object
     */
